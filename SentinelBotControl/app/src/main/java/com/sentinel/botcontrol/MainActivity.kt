...
class MainActivity : AppCompatActivity() {
    ...
    private var lastSelectedModeButton: Button? = null
    ...
    private fun handleIncomingLine(line: String) {
        if (line.startsWith("RADAR:")) {
            val payload = line.removePrefix("RADAR:")
            val parts = payload.split(",")
            if (parts.size == 2) {
                val angle = parts[0].trim().toIntOrNull()
                val dist = parts[1].trim().toIntOrNull()
    ...
    private fun rebuildLogText() {
        ...
        logEntries.forEachIndexed { index, entry ->
            val stamp = timeFmt.format(entry.time)
            val color = ContextCompat.getColor(this, typeColor(entry.type))
            val start = builder.length
            builder.append("${typeIcon(entry.type)} $stamp  ")
            ...
            if (index != logEntries.size - 1) builder.append("\n")
        }
        binding.logText.text = builder
    ...
    private fun bindModeButtons() {
        binding.btnNormal.setOnClickListener { send('n'); selectModeButton(binding.btnNormal) }
        binding.btnHappy.setOnClickListener { send('h'); selectModeButton(binding.btnHappy) }
        binding.btnSad.setOnClickListener { send('s'); selectModeButton(binding.btnSad) }
        binding.btnAngry.setOnClickListener { send('a'); selectModeButton(binding.btnAngry) }
        binding.btnGuard.setOnClickListener { send('g'); selectModeButton(binding.btnGuard) }
        binding.btnRescue.setOnClickListener { send('r'); selectModeButton(binding.btnRescue) }
        binding.btnFriendly.setOnClickListener { send('f'); selectModeButton(binding.btnFriendly) }
        binding.btnDanger.setOnClickListener { send('d'); selectModeButton(binding.btnDanger) }
        binding.btnLowbat.setOnClickListener { send('y'); selectModeButton(binding.btnLowbat) }
        binding.btnStuck.setOnClickListener { send('m'); selectModeButton(binding.btnStuck) }
    }

    private fun selectModeButton(button: Button) {
